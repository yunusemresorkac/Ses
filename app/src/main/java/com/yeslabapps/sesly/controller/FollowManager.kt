...
class FollowManager {
    ...
    fun followUser(userId: String, targetUserId: String,context : Context,viewModel: FirebaseViewModel) {
        ...
        followingCollection.document(targetUserId).get().addOnSuccessListener { snapshot ->
            if (snapshot.exists()) {
                ...
                followingCollection.document(targetUserId).set(mapOf("followed" to true)).addOnSuccessListener {
                    viewModel.getUserName(userId){ username ->
                        NotifyController().sendNotification(targetUserId,"$username Followed You.","New Follower!",userId,context)
                    }
                }
            }
        }
